...
def process_data(input_file: str,output_folder: str, output_file: str, search_str: str, pages: Tuple = None, action: str = 'Highlight'):
    """
    ...
    if total_matches > 0:
        with open(output_folder, mode='wb') as f:
...
def process_file(**kwargs):
    ...
    if action == "Remove":
        ...
        process_data(input_file=input_file,output_folder=output_folder, output_file=output_file,
...
def process_folder(**kwargs):
    ...
    #    if not os.path.isdir(input_folder+output_folder):
    #        os.mkdir(input_folder+output_folder)
    ...
    for foldername, dirs, filenames in os.walk(input_folder):
        for filename in filenames:
            ...
            process_file(input_file=inp_pdf_file,output_folder=output_folder+"/"+filename, output_file=inp_pdf_file,
...
def parse_args():
    ...
    parser.add_argument('-a', '--action', dest='action', choices=['Highlight', 'Remove'], type=str,
    ...
    if os.path.isdir(path):
        ...
        parser.add_argument('-O', '--output_folder', dest='output_path', type=str  # lambda x: os.path.has_valid_dir_syntax(x)
...
if __name__ == '__main__':
    ...
    args = parse_args()
    #os.mkdir(args['input_path']+"_highlighted")
    # If File Path
    ...
    # If Folder Path
        ...
        process_folder(
            ...
            search_str=args['search_str'] if 'search_str' in (args.keys()) else None, 
            action=args['action'], pages=args['pages'], recursive=args['recursive'], output_folder=args['output_path']
        )